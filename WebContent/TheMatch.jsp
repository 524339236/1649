				...
				<div>
					<table>
						...
						<tr>
							...
							<td nowrap="nowrap"><span>${match.matchStartTime } ~
									${match.matchEndTime }</span></td>
						</tr>
						<tr>
						...
						<c:choose>
							<c:when test="${match.matchSystem eq '团体赛'}">
								<tr>
									<td nowrap="nowrap"><a
										href="addteam.jsp?mid=${match.matchId }">创建队伍</a></td>
									<td nowrap="nowrap">
										<form action="jointeam.do?" name="teamId" id="teamId">
											<input type="text" id="teamId" name="teamId"
												placeholder="请输入队伍ID"> <input type="submit"
												value="加入队伍">
										</form>
									</td>
								</tr>
				...
				<div>
					...
					<table border="1px" width="80%" align="center">
						...
						<tr class="listheader">
							...
							<td width="15%" nowrap="nowrap"><div align="center">报名人身份
								</div></td>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${requestScope.joineduserList }"
									var="joineduser" varStatus="status">
									<!-- 这个是商品信息的显示行 -->
									<tr>
		...
		<c:when test="${requestScope.permission eq 'creater' }">
			...
			<div class="matchCreater">
				...
				<span><font style="bgcolor: #222c3a">添加/删除赛事管理员</font><a
					href="finduserinfo.do?op=setmanager&mid=${match.matchId }">➕/➖</a></span>


				<div class="left">
					<fieldset>
						<legend>比赛信息更改</legend>
						<form action="changematchdetail.do?op=changematch"
							...
							<table>
								<tr>
									...
									<td><input type="radio" name="matchtype" value="0">公有比赛<input
										type="radio" name="matchtype" value="1">私有比赛</td>
								</tr>
								<tr>
									<td>现定比赛赛制：</td>
								...
								<tr>
									<td>现定比赛时间：</td>
									<td><span>${match.matchStartTime } ~
											${match.matchEndTime }</span></td>

									<td><label class="xrequired">修改比赛时间：</label></td>
									<td><input type="date" min="2021-05-26"
										id="matchstarttime" name="matchstarttime"> ~ <input
										type="date" id="matchendtime" min="2021-05-26"
										name="matchendtime"></td>
								</tr>
								<tr>
				...
				<!-- 		比赛细节信息展示结束 		-->
					...
					<table style="align: center;" border="1px" width="80%"
						align="center">
						<tr>
				...
				<table border="1px" width="80%" align="center">
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach items="${requestScope.applyinguserList }"
								var="applyinguser" varStatus="status">
								<!-- 这个是商品信息的显示行 -->
								<tr>
									...
									<td nowrap="nowrap"><div align="center">
											<a
												href="matchmemberapply.do?uid=${applyinguser.userId }&mid=${match.matchId }&status=同意">同意</a>
											<a
												href="matchmemberapply.do?uid=${applyinguser.userId }&mid=${match.matchId }&status=拒绝">拒绝</a>
										</div></td>
								</tr>
								<!-- 这个是商品信息展示的结束 -->
							</c:forEach>
						...
						<c:otherwise>
							<c:forEach items="${requestScope.quitinguserList }"
								...
								<tr>
									...
									<td nowrap="nowrap"><div align="center">
											<a
												href="removeMember.do?uid=${quitinguser.userId }&mid=${match.matchId }&matchsystem=${match.matchSystem }">同意</a>
											<a
												href="matchmemberapply.do?uid=${quitinguser.userId }&mid=${match.matchId }&status=同意">拒绝</a>
										</div></td>
								</tr>
								<!-- 这个是商品信息展示的结束 -->
							</c:forEach>
